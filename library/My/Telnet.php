...
class My_Telnet {
	 ...
	 * @param array $prompt Connection timeout in seconds
	 * @return void
	 */
	public function __construct($host = '127.0.0.1', $port = '23', $timeout = 10, $prompt = array('$')){
	 ...
	 * @return boolean
	 */
	public function loginNetWorkElement($username, $password) {
		...
		try{
			...
			$this->waitPrompt();
			$this->write($password);
			$this->setPrompt('Windows NT Domain: ');
			$this->waitPrompt();
			$this->write("");
			$this->setPrompt(array('>','or>'));
			//Ejemplos de prompt recibidos:
			// c:\winnt\profiles\Administrator>
			// C:\WINNT\Profiles\Administrator.ANB05D>
			// C:\WINNT\Profiles\Administrator>
			$this->waitPrompt();
		} catch(Exception $e){
	 ...
	 * @return boolean
	 */
	public function loginEMA($username, $password) {
	
		try{
			$this->write("LOGIN:".$username.":".$password.";");
			$this->setPrompt('Enter command: ');
			$this->waitPrompt();
		} catch(Exception $e){
	 ...
	 * @return boolean
	 */
	protected function readTo($prompt_array) {
		if (! is_array($prompt_array)) {
			$prompt_array=array($prompt_array);
		}
		if (!$this->socket) {
		...
		do {
			...
			if (time() > $until_t) {
				throw new Exception("Couldn't find the requested : '.print_r($prompt_array,1).' within {$this->timeout} seconds");
			}

			$c = $this->getc();

			if ($c === false) {
				throw new Exception("Couldn't find the requested : '" . print_r($prompt_array,1) . "', it was not in the data returned from server: " . $this->buffer);
			}
			...
			if (is_array($prompt_array)) {
				foreach ($prompt_array as $k => $prompt)	{
					$posible_prompt=(substr($this->buffer, strlen($this->buffer) - strlen($prompt)));
					if ($posible_prompt == $prompt){
						return self::TELNET_OK;
					}
				}		
			}
			//My_Utils::log("buf:".$this->buffer);
